...
$(document).ready(function() {	
	...
	$( "#forward_msg" ).dialog({
		...
		buttons: {
			"Forward": function() {
				...
				var opts = { context: "propagate", propagate_type: "forward", callback: propagateMsg };
	...
	$('#threaded_view_toggle').bt({contentSelector: "$('#threaded_view_toggle_help').html()",positions: ['right','top'],trigger: 'hoverIntent', width: '350px', hoverIntentOpts: {interval: 850} });
	$('#notes_flag_toggle').bt({contentSelector: "$('#notes_flag_toggle_help').html()",positions: ['right','top'],trigger: 'hoverIntent', width: '350px', hoverIntentOpts: {interval: 850} });
	$('#notes_flag_bmrb_toggle').bt({contentSelector: "$('#notes_flag_bmrb_toggle_help').html()",positions: ['right','top'],trigger: 'hoverIntent', width: '350px', hoverIntentOpts: {interval: 850} });
	...
	$('#archive_msg_ok').on('click', function() {
		var msgId = MsgingMod.sCrrntMsgIdSlctd;
		var opts = { context: "propagate", propagate_type: "archive", callback: propagateMsg };
		getMsgDict(msgId,opts);
	});
	...
	$('.reply').on('click', function(){
		...
		var opts = {context: "compose", msg_prefix: "RE: ", callback: composeMsg};
		getMsgDict(msgId,opts);
	});
 ...
 *		
 *****************************************************************************/
	$('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.CHECK_GLBL_MSG_STATUS, async: true, clearForm: false,
        success: function(jsonData) {
        	if( jsonData ){
        		...
        		if( MsgingMod.bNotesExist ){
        			...
        			$("#get_content_type_notes").bt('There are notes present for this deposition.',{positions: ['top', 'right'],trigger: 'hoverIntent', hoverIntentOpts: {interval: 850} });
        		}else{
        			$("#get_content_type_notes").bt('There are currently no notes for this deposition.',{positions: ['top', 'right'],trigger: 'hoverIntent', hoverIntentOpts: {interval: 850} });
        		}
 ...
 *****************************************************************************/
	...
	$('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.TOGGLE_NOTES_FLAGGING, async: true, clearForm: false,
        beforeSubmit: function (formData, jqForm, options) {
			formData.push( {"name": paramName, "value": paramValue} );
			if( subTypeRqstd ){
				formData.push( {"name": "subtype", "value": "bmrb"} );
			}
    	},
        success: function(jsonData) {
    		checkGlobalMsgStatus();
...
function getFilesReferenced(){
	 ...
	 */
	$('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.GET_FILES_RFRNCD, async: true, clearForm: false,
        success: function(jsonData) {
        	if( jsonData ){
...
function confirmAvailFiles(callbackFn){
	$('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.CHECK_AVAIL_FILES, async: true, clearForm: false,
        success: function(jsonData) {
        	if( jsonData ){
 ...
 **/
    ...
    $('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.DTBL_GET_TMPLT, async: true, clearForm: false,
        dataType: 'json',
        beforeSubmit: function (formData, jqForm, options) {
    		formData.push( {"name": "serverside", "value": MsgingMod.sUseServerSidePrcssing} );
        	formData.push( {"name": "usethreaded", "value": MsgingMod.sUseThreaded} );
				...
				***/
				...
				var oTable = $('#'+MsgingMod.sDepId+'_tbl').dataTable({
                    ...
                    "fnServerParams": function ( aoData ) {
						...
						aoData.push( {"name": "identifier", "value": MsgingMod.sDepId } );
						aoData.push( {"name": "sessionid", "value": MsgingMod.sSessionId} );
						aoData.push( {"name": "filesource", "value": MsgingMod.sFileSource} );
						aoData.push( {"name": "serverside", "value": MsgingMod.sUseServerSidePrcssing} );
						aoData.push( {"name": "usethreaded", "value": MsgingMod.sUseThreaded} );
						aoData.push( {"name": "send_status", "value": sendStatusToUse} );
						aoData.push( {"name": "content_type", "value": MsgingMod.sCrrntContentType} );
			        ...
			        "aaSorting": [[ convertColNameToIntIndex('timestamp'), "desc" ]],
					"aoColumnDefs": [
										/***
										{
			...
			if( jsonData.dtbl_config_dict ){
				...
				var oTable = $('#'+MsgingMod.sDepId+'_tbl').dataTable({
                    ...
                    "fnServerParams": function ( aoData ) {
						...
						aoData.push( {"name": "serverside", "value": MsgingMod.sUseServerSidePrcssing} );
						aoData.push( {"name": "usethreaded", "value": MsgingMod.sUseThreaded} );
						aoData.push( {"name": "send_status", "value": "Y"} );
						aoData.push( {"name": "content_type", "value": MsgingMod.sCrrntContentType} );
...
function displayDraftMsg(thisRow){
    ...
    if( oDraftMsg.parent_message_id != oDraftMsg.message_id ){
    	var opts = {context: "compose", msg_subject: oDraftMsg.message_subject, msg_text: sMsgText, callback: composeMsg};
...
function previewMsg(thisRow){
    ...
    if( oMsg.parent_message_id != oMsg.message_id ){
    	var opts = { context: "display", display_type: "preview_pane"};
    ...
    $("#preview_msg").css({overflow:'auto', height:'99%'});
...
function displayMsg(thisRow,bUseNewWindow){
    ...
    if( oMsg.parent_message_id != oMsg.message_id ){
    	var opts = { context: "display", display_type: "dialog"};
        getMsgDict( oMsg.parent_message_id, opts);
    }else{
		$('#parent_msg').html("");
		$('#parent_msg_div').hide();
...
function getMsgDict(msgId,opts){
	var oMsg = new Message();
	$('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.GET_MSG, async: true, clearForm: false,
        dataType: 'json',
        beforeSubmit: function (formData, jqForm, options) {
...
function markMsgAsRead(msgId) {
	...
	$('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.MARK_MSG_READ, async: true, clearForm: false,
        beforeSubmit: function (formData, jqForm, options) {
        	formData.push({"name": "msg_id", "value": msgId});
        },
        success: function() {
...
function tagMsg(actionReqd,readStatus,forRelease) {
	...
	$('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.TAG_MSG, async: true, clearForm: false,
        beforeSubmit: function (formData, jqForm, options) {
        	formData.push({"name": "msg_id", "value": MsgingMod.sCrrntMsgIdSlctd});
...
function submitNewMsg(sendStatusToUse) {
	...
	$('#msg_compose_frm').ajaxSubmit({url: sUrl, async: true, clearForm: false,
		beforeSubmit: function( formData, jqForm, options ){
...
function propagateMsg( msgSubject, parentMsgId, parentMsg, parentMsgSnder, parentMsgDateTime, actionType ){
	...
	$('#hlprfrm').ajaxSubmit({url: sUrl, async: true, clearForm: false, type: 'POST',
		beforeSubmit: function( formData, jqForm, options ){
			//formData.push({"name": "parent_msg", "value": $('#msg_compose_parent_msg').html()});
			formData.push({"name": "send_status", "value": "Y"});
			formData.push({"name": "filesource", "value": MsgingMod.sFileSource} );
...
function loadTestExampleSeeJson(depDataSetID) {
	$('#hlprfrm').ajaxSubmit({url: MsgingMod.URL.SEE_RAW_JSON_FOR_DTBL, async: true, clearForm: false,
        target: '#rslts',
        beforeSubmit: function (formData, jqForm, options) {
        	formData.push({"name": "identifier", "value": depDataSetID});
        },
        success: function() {
			// PLACEHOLDER
        }
